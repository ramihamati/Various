...
var message = new Message { Content = "Hello, Server 2!", Timestamp = DateTime.Now };
    ...
    public async Task ReadAsync(NetworkStream stream)
    {
        ...
        while (true)
        {
            ...
            foreach (var segment in buffer)
            {
                ...
                var message = new Message { Content = "Hello, Server 2!", Timestamp = DateTime.Now };
                var messageString1 = $"{message.Content} - {message.Timestamp}";
                var messageBytes1 = Encoding.UTF8.GetBytes(messageString);
                await stream.WriteAsync(messageBytes1, 0, messageBytes1.Length);
            }
